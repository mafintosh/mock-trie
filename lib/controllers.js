...
class IteratorController {
  ...
  pop () {
    let { d, key, i, val, head, symlinks, target } = this.stack.pop()

    for (; i < head.trie.length; i++) {
      const links = head.trie[i] || []

      if (((i + 1) & 31) === 0) {
        this._push(i, head, key, false)
        this._push(i, head, target, false)
      }
      ...
      for (; val < links.length; val++) {
        ...
        const seq = links[val]
        if (!seq) continue
        this.stack.push({ d, symlinks, key, i, val: val + 1, head, target })
        this.stack.push({ d, symlinks: symlinks.slice(), key: key.slice(), target: target.slice(), i: i + 1 + 32 * offset, val: 0, head: this.getSeq(seq) })
        return null
      }
    ...
    if ((i & 31) !== 0 || i < head.hash.length) {
      this._push(i, head, key, true)
      this._push(i, head, target, true)
    }
    ...
    if (value.symlink) {
      ...
      if (symlinks.includes(id)) {
        return null
      }

      symlinks.push(id)

      const k = key.join('/') //this.handlers.realpath(key.join('/'))

      if (d >= MAX_SYMLINK_DEPTH) {
        return null
      }

      // console.log(target, k)
      // const t = this.handlers.realpath(k)
      // if (!t) return null
      const resolved = resolveLink(target.join('/'), target.join('/'), value.symlink, p => {
        const r = this.handlers.realpath(p)
        return r && r.key
      })
      ...
      if (this.handlers && this.handlers.get) {
        // console.log({ visited: symlinks, skip: resolveLink.directories })
        const r = this.handlers.get(resolved, { visited: symlinks })
        if (!r) return null
        let { i, node: head, depth, offset, target } = r

        i += offset

        if (k.depth + depth >= MAX_SYMLINK_DEPTH) {
          return null
        }

        if (head) {
          this.stack.push({ d: d + 1, symlinks, key, i, val: 0, head: this.getSeq(head.seq), target: target.split('/') })
        }
        return null
      }
    }
  ...
  next () {
    if (!this.stack) {
      if (this.handlers.prefix) {
        const p = this.handlers.prefix //this.handlers.realpath(this.handlers.prefix)
        const r = p && this.handlers.get(p)

        if (r) {
          let { i, node: head, offset, target } = r
          i += offset
          if (!head) this.stack = []
          else this.stack = [{ d: 0, symlinks: [], key: this.handlers.prefix.split('/'), i, val: 0, head: this.getSeq(head.seq), target: target.split('/') }]
        } else {
          this.stack = []
        }
      } else {
        this.stack = [{ d: 0, symlinks: [], key: [], i: 0, val: 0, head: this.getSeq(this.feed.length - 1), target: [] }]
      }
    }
...
class GetController {
  ...
  update () {
    if (!this.target.key.length && this.handlers.closest) {
      return { node: this.getSeq(this.feed.length - 1), feed: this.feed, i: 0, offset: 0 }
    }
    ...
    if (this.handlers.onclosest) {
      if (this.i >= len) this.prev = { node: this.head, i: this.i, feed: this.feed, offset: this._o, target: this.target.key.toString() }
      this.head = this.handlers.onclosest(this.head)
      if (!this.head)
      if (this._reset) {
        this._reset = false
    ...
    if (this.head && this.i >= len) {
      ...
      return { node: this.head, feed: this.feed, i: this.i, offset: this._o, target: this.target.key.toString() }
    }
    ...
    return { node: null, feed: null, i: 0, offset: this._o, target: this.target.key.toString() }
  }
