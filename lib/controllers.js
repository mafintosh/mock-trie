...
class IteratorController {
  ...
  pop () {
    let { d, key, i, val, head, symlinks } = this.stack.pop()
    ...
    for (; i < head.trie.length; i++) {
      ...
      for (; val < links.length; val++) {
        ...
        const seq = links[val]
        if (!seq) continue
        this.stack.push({ d, symlinks, key, i, val: val + 1, head })
        this.stack.push({ d, symlinks: symlinks.slice(), key: key.slice(), i: i + 1 + 32 * offset, val: 0, head: this.getSeq(seq) })
        return null
      }
    ...
    if (value.symlink) {
      ...
      const target = this.handlers.realpath(k)
      if (!target) return null
      const resolved = this.handlers.realpath(resolveLink(target.key, target.key, value.symlink)).key
      if (!resolved) return null
        // console.log('resolve', target.key, k, value.symlink, resolved)

      if (this.handlers && this.handlers.get) {
        const r = this.handlers.get(resolved)
        if (!r) return null
        let { i, node: head, depth, offset } = r
        // console.log(k, resolved, i, head)


        i += offset

        // if (head && symlinks.includes(head.seq)) {
        //   console.log('?', key, resolved)
        //   return null
        // }
// console.log('___', symlinks, i, resolved, head)
        if (k.depth + depth >= MAX_SYMLINK_DEPTH) {
          return null
        }

        if (head) {
          this.stack.push({ d: d + 1, symlinks, key, i, val: 0, head: this.getSeq(head.seq) })
        }
        return null
      }
    }
  ...
  next () {
    if (!this.stack) {
      if (this.handlers.prefix) {
        const p = this.handlers.realpath(this.handlers.prefix)
        const r = p && this.handlers.get(p.key)

        if (r) {
          let { i, node: head, offset } = r
          i += offset
          if (!head) this.stack = []
          else this.stack = [{ d: 0, symlinks: [], key: this.handlers.prefix.split('/'), i, val: 0, head: this.getSeq(head.seq) }]
        } else {
          this.stack = []
        }
      } else {
        this.stack = [{ d: 0, symlinks: [], key: [], i: 0, val: 0, head: this.getSeq(this.feed.length - 1) }]
      }
    }
...
class GetController {
  ...
  update () {
    ...
    if (this.handlers.onclosest && !this.handlers.closest) {
      if (this.i >= len) this.prev = { node: this.head, i: this.i, feed: this.feed, offset: this._o }
      this.head = this.handlers.onclosest(this.head)
      if (!this.head)
      if (this._reset) {
        this._reset = false
    ...
    if (this.head && this.i >= len) {
      ...
      return { node: this.head, feed: this.feed, i: this.i, offset: this._o }
    }
    ...
    return { node: null, feed: null, i: 0, offset: this._o }
  }
